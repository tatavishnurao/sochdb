...
fn brute_force_knn(query: &[f32], vectors: &[(u128, Vec<f32>)], k: usize) -> Vec<(u128, f32)> {
...
fn test_batch_vs_sequential_parity() {
    ...
    println!(
        "Recall difference: {:.2}%",
        (batch_avg_recall - seq_avg_recall).abs() * 100.0
    );
    ...
    assert!(
        batch_avg_recall >= 0.85,
        "Batch recall too low: {:.2}%",
        batch_avg_recall * 100.0
    );
    assert!(
        seq_avg_recall >= 0.85,
        "Sequential recall too low: {:.2}%",
        seq_avg_recall * 100.0
    );
}
...
fn test_simple_sequential_insert() {
    ...
    for (id, vec) in &vectors {
        ...
        if results.is_empty() {
            ...
            if failures < 10 {
                println!(
                    "FAIL: Query {} returned {} with distance {}",

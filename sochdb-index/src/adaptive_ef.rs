...
impl AdaptiveEfController {
    ...
    pub fn compute_adaptive_ef(&self, is_batch: bool) -> usize {
        ...
        let ef = match workload {
            ...
            WorkloadType::MatureGraph => {
                ...
                ((self.config.base_ef as f32 * reduction_factor) as usize).max(self.config.min_ef)
            }
    ...
    pub fn update_ef(&self, is_batch: bool) -> usize {
        ...
        if new_ef != old_ef {
            ...
            eprintln!(
                "[AdaptiveEf] Updated ef: {} -> {} (workload: {:?})",
                old_ef,
                new_ef,
    ...
    pub fn record_insert(&self, connections_added: usize) {
        ...
        self.connection_count
            .fetch_add(connections_added as u64, Ordering::Relaxed);
    ...
    fn is_quality_critical(&self) -> bool {
        ...
        let recent_measurements: Vec<_> = quality_history
            .iter()
        ...
        let avg_quality: f32 = recent_measurements
            .iter()
    ...
    pub fn stats(&self) -> AdaptiveEfStats {
        ...
        let avg_quality = if !quality_history.is_empty() {
            quality_history.iter().map(|m| m.recall_at_10).sum::<f32>()
                / quality_history.len() as f32
        } else {
            0.0
        };

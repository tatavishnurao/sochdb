...
impl ContiguousNodeStorage {
    ...
    pub fn get_mut(
        &self,
        index: u32,
    ) -> Option<parking_lot::MappedRwLockWriteGuard<'_, CompactHnswNode>> {
        ...
        let mapped =
            parking_lot::RwLockWriteGuard::map(nodes, move |vec: &mut Vec<CompactHnswNode>| {
    ...
    pub fn memory_usage(&self) -> usize {
        ...
        let id_map_memory =
            id_map.len() * (std::mem::size_of::<u128>() + std::mem::size_of::<u32>());
        ...
        // Add neighbor data
        let neighbor_memory: usize = nodes
            .iter()
            .map(|node| {
                node.layers
                    .iter()

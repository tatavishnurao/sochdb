...
impl ProjectionMatrix {
    ...
    pub fn project_into(&self, vector: &[f32], output: &mut [f32]) {
        ...
        for (i, row) in self.matrix.iter().enumerate() {
            output[i] = row.iter().zip(vector.iter()).map(|(a, b)| a * b).sum();
        }
    }
...
impl ProjectedVectors {
    ...
    pub fn memory_usage(&self) -> usize {
        let projections_mem =
            self.projections.len() * DEFAULT_PROJECTION_DIM * std::mem::size_of::<f32>();
...
impl JLFilter {
    ...
    pub fn new(original_dim: usize) -> Self {
        let projection_matrix =
            Arc::new(ProjectionMatrix::new(original_dim, DEFAULT_PROJECTION_DIM));

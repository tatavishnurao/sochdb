...
pub enum ArrowIpcError {
    ...
    #[error(
        "Invalid schema: expected 'id' (UInt64) and 'vector' (FixedSizeList<Float32>), got: {0}"
    )]
...
/// ```
pub fn insert_from_arrow_ipc(index: &HnswIndex, ipc_data: &[u8]) -> Result<usize, ArrowIpcError> {
    ...
    for batch_result in reader {
        ...
        let values = vector_list
            ...
            .ok_or_else(|| ArrowIpcError::InvalidSchema("vector values are not Float32".into()))?;
        ...
        let ids: Vec<u128> = (0..num_rows).map(|i| id_array.value(i) as u128).collect();

        // Call the flat batch insert API
        let inserted = index
            .insert_batch_flat(&ids, raw_floats, index_dim)
...
pub fn create_vector_batch(
    ...
    use arrow::array::{ArrayRef, FixedSizeListArray, Float32Array, RecordBatch, UInt64Array};
...
pub fn serialize_to_ipc(batch: &arrow::array::RecordBatch) -> Result<Vec<u8>, ArrowIpcError> {
...
mod tests {
    ...
    fn test_dimension_mismatch() {
        ...
        assert!(matches!(
            result,
            Err(ArrowIpcError::DimensionMismatch { .. })
        ));
    }

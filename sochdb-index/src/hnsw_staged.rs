...
use crate::hnsw::{DistanceMetric, HnswIndex, HnswNode, VersionedNeighbors};
...
impl<'a> StagedBuilder<'a> {
    ...
    pub fn insert_batch(
        ...
        let scaffold_threshold = self
            .config
            .scaffold_size
            .max(2 * self.index.config.max_connections_layer0);
        ...
        for wave_idx in 0..num_waves {
            ...
            let wave_results: Vec<_> = wave_batch
                .par_iter()
                .map(|(id, vector)| self.insert_with_deferred_backedges(*id, vector))
                .collect();
    ...
    fn calculate_distance(&self, a: &QuantizedVector, b: &QuantizedVector) -> f32 {
        match self.index.config.metric {
            ...
            DistanceMetric::Cosine => crate::vector_quantized::cosine_distance_quantized(a, b),
            DistanceMetric::DotProduct => crate::vector_quantized::dot_product_quantized(a, b),
        }
    }
        ...
        for candidate in sorted {
            ...
            let is_shadowed = result.iter().any(|selected: &SearchCandidate| {
                if let (Some(c_node), Some(s_node)) = (
                    ...
                    let c_vec = vector_store_guard
                        .get(c_node.vector_index as usize)
                        .unwrap_or(&c_node.vector);
                    let s_vec = vector_store_guard
                        .get(s_node.vector_index as usize)
                        .unwrap_or(&s_node.vector);

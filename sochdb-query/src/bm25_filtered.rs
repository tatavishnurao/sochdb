...
impl<I: InvertedIndex> FilteredBm25Executor<I> {
    ...
    /// 6. Return top-k
    pub fn search(&self, query: &str, k: usize, allowed: &AllowedSet) -> Vec<ScoredResult> {
...
impl<I: PositionalIndex> FilteredPhraseExecutor<I> {
    ...
    pub fn search(&self, phrase: &[&str], k: usize, allowed: &AllowedSet) -> Vec<ScoredResult> {
        ...
        let results: Vec<ScoredResult> = candidates
            ...
            .filter_map(|(doc_id, phrase_freq)| {
                ...
                let min_df = positional_postings
                    .iter()
...
mod tests {
    ...
    impl MockIndex {
        fn new() -> Self {
            ...
            postings.insert(
                "rust".to_string(),
                PostingList::new("rust", vec![(1, 3), (2, 1), (3, 2), (5, 1)]),
            );
            postings.insert(
                "database".to_string(),
                PostingList::new("database", vec![(1, 1), (3, 4), (4, 1)]),
            );
            postings.insert(
                "vector".to_string(),
                PostingList::new("vector", vec![(1, 2), (2, 3), (4, 1), (5, 2)]),
            );

...
use sochdb_tools::io::{FaultTelemetry, ensure_resident_for_hnsw, load_bulk};
use sochdb_tools::io::{VectorFormat, ids_from_mmap, open_ids, open_vectors};
use sochdb_tools::ordering::{OrderingStrategy, compute_ordering};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Info { index } => show_info(index),
        Commands::Convert {
            input,
            output,
            from_format,
...
fn build_index(
    ...
    let (n, d, vectors, _owned_data): (usize, usize, &[f32], Option<sochdb_tools::OwnedVectors>) =
        if direct_read {
            eprintln!("Loading vectors via direct read (--direct-read)...");
            let owned = load_bulk(&input, dimension).context("Failed to load vectors")?;
            let n = owned.num_vectors;
            let d = owned.dimension;
            eprintln!(
                "  Loaded {} vectors × {} dimensions ({:.1} MB)",
                n,
                d,
            ...
            let reader =
                open_vectors(&input, format, dimension).context("Failed to open vector file")?;
    ...
    let (vectors, ids): (&[f32], Vec<u128>) = if ordering_strategy != OrderingStrategy::None {
        ...
        let strategy = match ordering_strategy {
            OrderingStrategy::CoarseKMeans { .. } => OrderingStrategy::CoarseKMeans {
        ...
        eprintln!(
            "  Compute time: {:.2}s",
            reorder_result.compute_time.as_secs_f64()
        );
    ...
    for (_batch_idx, chunk_start) in (0..n).step_by(batch_size).enumerate() {
        ...
        let inserted = index
            .insert_batch_contiguous(batch_ids, batch_vectors, d)
    ...
    let file_size = {
        ...
        index
            .save_to_disk_compressed(&output)
...
fn load_index(path: &PathBuf) -> Result<HnswIndex> {
    ...
    HnswIndex::load_from_disk(path).map_err(|e| anyhow::anyhow!("Failed to load index: {}", e))
}
...
fn query_index(
    ...
    let query: &[f32] =
        unsafe { std::slice::from_raw_parts(query_data.as_ptr() as *const f32, stats.dimension) };

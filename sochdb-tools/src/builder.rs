...
pub enum BuildError {
    #[error(
        "Dimension mismatch: vectors length {vectors_len} not divisible by dimension {dimension}"
    )]
    DimensionMismatch {
        vectors_len: usize,
        dimension: usize,
...
pub fn build_hnsw_index(
    ...
    for chunk_start in (0..n).step_by(batch_size) {
        ...
        let inserted = index
            .insert_batch_contiguous(batch_ids, batch_vectors, dim)

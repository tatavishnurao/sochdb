...
pub fn load_vectors_bulk(path: &Path, dimension: usize) -> Result<OwnedVectors, ToolsError> {
    ...
    let bytes: &mut [u8] =
        unsafe { std::slice::from_raw_parts_mut(data.as_mut_ptr() as *mut u8, file_size) };
...
pub fn load_vectors_bulk_exact(
    ...
    // Read exact amount
    let bytes: &mut [u8] =
        unsafe { std::slice::from_raw_parts_mut(data.as_mut_ptr() as *mut u8, expected_size) };
...
pub fn load_npy_bulk(path: &Path) -> Result<OwnedVectors, ToolsError> {
    ...
    let bytes: &mut [u8] =
        unsafe { std::slice::from_raw_parts_mut(data.as_mut_ptr() as *mut u8, n_floats * 4) };
    ...
    match ext {
        ...
        "f32" | "bin" | "raw" => {
            let dim = dimension.ok_or_else(|| {
                ToolsError::InvalidArgument("Dimension required for raw format".to_string())
            })?;

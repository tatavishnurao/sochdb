...
impl FaultTelemetry {
    ...
    pub fn delta(&self) -> Option<FaultStats> {
        self.end_stats
            .as_ref()
            .map(|end| end.delta_from(&self.start_stats))
    }
    ...
    pub fn print_summary(&self, label: &str) {
        ...
        eprintln!(
            "║  Max RSS:        {:>10.1} MB                               ║",
            delta.max_rss_kb as f64 / 1024.0
        );
        eprintln!(
            "║  User CPU:       {:>10.3}s                                 ║",
            delta.user_time_us as f64 / 1e6
        );
        eprintln!(
            "║  Sys CPU:        {:>10.3}s                                 ║",
            delta.sys_time_us as f64 / 1e6
        );
        eprintln!("╚══════════════════════════════════════════════════════════════╝");
        ...
        if sys_ratio > 0.5 {
            eprintln!();
            eprintln!(
                "⚠️  HIGH SYSTEM TIME RATIO: {:.1}% sys vs user",

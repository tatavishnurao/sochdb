...
impl NpyReader {
    ...
    pub fn from_mmap(mmap: Mmap) -> Result<Self, ToolsError> {
        ...
        let (header_len, header_start) = match version_major {
            ...
            2 | 3 => {
                if mmap.len() < 12 {
                    return Err(ToolsError::InvalidNpyHeader(
                        "File too small for v2 header".to_string(),
                    ));
                }
            ...
            _ => {
                return Err(ToolsError::InvalidNpyHeader(format!(
                    "Unsupported version: {}.{}",
        ...
        if mmap.len() < data_offset {
            return Err(ToolsError::InvalidNpyHeader(
                "Header extends past file end".to_string(),
            ));
        }
        ...
        if descr != "<f4" && descr != "|f4" && descr != "float32" {
            return Err(ToolsError::InvalidNpyHeader(format!(
                "Only float32 dtype supported, got '{}'",
                descr
            )));
        }
    ...
    fn extract_field(header: &str, field: &str) -> Result<String, ToolsError> {
        ...
        Err(ToolsError::InvalidNpyHeader(format!(
    ...
    fn extract_shape(header: &str) -> Result<(usize, usize), ToolsError> {
        ...
        let paren_start = rest.find('(').ok_or_else(|| {
            ToolsError::InvalidNpyHeader("Could not find shape tuple".to_string())
        })?;
        let paren_end = rest.find(')').ok_or_else(|| {
            ToolsError::InvalidNpyHeader("Could not find shape tuple end".to_string())
        })?;
        ...
        match dims.len() {
            ...
            2 => Ok((dims[0], dims[1])),
            _ => Err(ToolsError::InvalidNpyHeader(format!(
                "Expected 1D or 2D array, got {} dimensions",
                dims.len()
            ))),
        }
    }

...
use sochdb_vector::simd::{bps_scan, dispatch, dot_i8, visibility};
...
fn bench_dot_i8_batch(c: &mut Criterion) {
    ...
    for n_vec in [10, 100, 1000] {
        ...
        group.bench_with_input(BenchmarkId::new("batch", n_vec), &n_vec, |bench, _| {
            bench.iter(|| {
                dot_i8::dot_i8_batch(
                    black_box(&query),
                    black_box(&vectors),
...
fn bench_visibility(c: &mut Criterion) {
    ...
    for n_rows in [100, 1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::new("basic", n_rows), &n_rows, |bench, _| {
            bench.iter(|| {
                visibility::visibility_check(black_box(&commits), snapshot, black_box(&mut mask))
            })
        });
    }
...
fn print_cpu_info(c: &mut Criterion) {
    ...
    group.bench_function("detect", |b| b.iter(|| dispatch::simd_level()));

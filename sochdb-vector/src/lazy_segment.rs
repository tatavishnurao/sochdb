        ...
        let codes: Vec<u64> = (0..num_vectors)
            .map(|i| {
                ...
                for (p, proj) in projections.iter().enumerate().take(64) {
                    let dot: f32 = vector.iter().zip(proj.iter()).map(|(a, b)| a * b).sum();
...
impl LazySegment {
    ...
    pub fn from_flat(flat_data: Vec<f32>, keys: Vec<VectorKey>, config: LazyConfig) -> Self {
        let key_to_index: HashMap<_, _> = keys
            .iter()
            .enumerate()
            .map(|(i, &k)| (k, i as u32))
            .collect();
    ...
    fn brute_force_search(&self, query: &[f32], k: usize) -> Vec<(VectorKey, f32)> {
        ...
        let mut results: Vec<(VectorKey, f32)> = self
            .index_to_key
            .iter()
            .enumerate()
            .map(|(i, &key)| {
    ...
    fn bps_candidates(&self, query: &[f32], limit: usize) -> Vec<u32> {
        ...
        let mut scored: Vec<(u32, u32)> = bps
            .codes
            .iter()
            .enumerate()
    ...
    fn rdf_candidates(&self, query: &[f32], limit: usize) -> Vec<u32> {
        ...
        let mut scored: Vec<(u32, u32)> = rdf
            .quantized
            .iter()
            .enumerate()
    ...
    pub fn prewarm(&self) {
        ...
        if self.config.enable_graph {
            let _ = self
                .indexes
                .graph
                .get_or_init(|| MiniGraph::build(&self.data, self.config.dim, self.config.graph_m));
        }
    }
...
impl LazySegmentManager {
    ...
    pub fn search(&self, query: &[f32], k: usize) -> Vec<(VectorKey, f32)> {
        ...
        let mut results: Vec<(VectorKey, f32)> =
            segments.iter().flat_map(|s| s.search(query, k)).collect();
...
mod tests {
    ...
    fn test_segment_manager() {
        ...
        // Add segments
        let seg1 = LazySegment::new(vec![(1, vec![1.0, 0.0, 0.0, 0.0])], config.clone());
        let seg2 = LazySegment::new(vec![(2, vec![0.0, 1.0, 0.0, 0.0])], config.clone());
    ...
    fn test_bps_index() {
        let dim = 8;
        let vectors: Vec<f32> = (0..100 * dim).map(|i| (i as f32 * 0.01).sin()).collect();
        ...
        // Querying with a stored vector must reproduce that vector's stored code
        // (build and query share the same projection logic — it is deterministic).
        let first_vec = vectors[0..dim].to_vec();
        let code = bps.query(&first_vec);
        assert_eq!(
            code, bps.codes[0],
            "query of a stored vector must match its build-time code"
        );

        // Determinism: the same query must always yield the same code.
        assert_eq!(code, bps.query(&first_vec), "query must be deterministic");

        // Only the low `num_projections` (32) bits may be set.
        assert_eq!(
            code & !((1u64 << 32) - 1),
            0,
            "no bits beyond num_projections"
        );

        // A code is identical to itself under Hamming distance.
        assert_eq!(BpsIndex::hamming_distance(code, code), 0);
    }

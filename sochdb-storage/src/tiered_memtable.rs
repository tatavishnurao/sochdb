...
impl SortedBatch {
    ...
    pub fn prefix_scan(&self, prefix: &[u8]) -> impl Iterator<Item = &HotEntry> {
        ...
        let start_idx = self.entries.partition_point(|e| e.key.as_slice() < prefix);
...
impl TieredMemTable {
    ...
    pub fn write(&self, key: &[u8], value: Option<Vec<u8>>, txn_id: u64) -> Result<()> {
        ...
        self.size_bytes
            .fetch_add((key.len() + value_size) as u64, Ordering::Relaxed);
    ...
    pub fn scan_prefix_tournament(
        ...
        // Source 0: Sorted hot buffer entries matching prefix
        {
            ...
            // Sort by key, then by seq desc (newest first for same key)
            hot_entries.sort_unstable_by(|a, b| match a.key.as_slice().cmp(b.key.as_slice()) {
        ...
        // Sources 1..K: Warm batches (already sorted)
        {
            ...
            for batch in batches.iter().rev() {
                let entries: Vec<HotEntry> = batch.prefix_scan(prefix).cloned().collect();
        ...
        let iters: Vec<_> = sorted_sources
            ...
            .enumerate()
            .map(|(source_idx, v)| {
                v.into_iter().map(move |e| KeyedEntry {
                    entry: e,
                    source_idx,
                })
            })
    ...
    pub fn compact(&self) -> Result<()> {
        ...
        let all_entries: Vec<HotEntry> = batches.iter().flat_map(|b| b.iter().cloned()).collect();
...
mod tests {
    ...
    fn test_scan_prefix_tournament() {
        ...
        for batch_idx in 0..3 {
            for i in 0..10 {
                ...
                table
                    .write(key.as_bytes(), Some(value.into_bytes()), 1)
                    .unwrap();
            }
            table.flush().unwrap();
        }
    ...
    fn test_scan_tournament_deduplication() {
        ...
        table
            .write(b"key:001", Some(b"newest".to_vec()), 1)
            .unwrap();

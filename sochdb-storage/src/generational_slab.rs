...
impl SlotHeader {
    ...
    fn set_allocated(&self, allocated: bool) {
        if allocated {
            ...
            self.flags
                .fetch_and(!Self::FLAG_ALLOCATED, Ordering::Release);
        }
    }
...
impl SlabAllocator {
    ...
    pub fn with_config(config: SlabAllocatorConfig) -> Self {
        let slabs = std::array::from_fn(|i| {
            ...
            let slab = Slab::new(size_class, initial_slots).expect("Failed to create initial slab");
    ...
    pub fn allocate_from_class(
        &self,
        size_class: SizeClass,
    ) -> Option<(GenerationalHandle, NonNull<u8>)> {
...
impl<T: Sized> TypedSlabAllocator<T> {
    ...
    ///
    /// # Safety
    /// The caller must ensure exclusive access for `handle`: no other live
    /// reference (shared or mutable) to the same slot may exist for the
    /// returned borrow's lifetime. The slab hands out interior references
    /// through `&self`, so aliasing is the caller's contract to uphold.
    // `mut_from_ref`: deliberate interior-mutability slab access, guarded by the
    // Safety contract above (mirrors the unsafe arena accessors in sochdb-index).
    #[allow(clippy::mut_from_ref)]
    pub unsafe fn get_mut(&self, handle: GenerationalHandle) -> Option<&mut T> {
        let ptr = self.allocator.get_ptr(self.size_class, handle)?;
